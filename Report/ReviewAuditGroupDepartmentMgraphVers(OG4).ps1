...
# Prompt departement available
$SearchDep = Get-MgUser -All -property Department| Select-Object Department -Unique
foreach($dep in $SearchDep){
    Write-Host $dep.Department
}
Write-Host "Enter the department you want to choose" -ForegroundColor DarkYellow
...
foreach ($group in $AllGroup) {
    ...
    $devices = $members | Where-Object { $_.ObjectType -eq 'device' -or $_.ObjectType -eq "Group" -or $_.ObjectType -eq "Service Principal" }
    if ($members.Count -eq 0 -and $devices.Count -eq 0) {
        $groupsNoMemberAndDevice += $group}

    $owners = Get-MgGroupOwner -GroupId $group.Id
    if ($owners.Count -eq 0 -and $members.Count -eq 0 -or $devices.Count -eq 0) {
        $groupsNoOwner += $group
    }
}
