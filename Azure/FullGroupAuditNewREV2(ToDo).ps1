...
$outputFilePath = "C:\Users\GregorySemedo\Desktop\Only-Marketing.csv"
$sharedMailboxFilePath = "C:\Users\GregorySemedo\Desktop\SharedMailboxOnly-Marketing.csv"
...
foreach ($user in $NolicenceCSV) {
    ...
    $userInfo = Get-MsolUser -ObjectId $user.User
    
    # Check if user has a licence
    if (!$userInfo.Licenses) {
...
$UserList = Import-Csv -Path "C:\Test\Only-Marketing-SharedMailbox.csv"
