...
$ExternalUser = Get-MgUser -All | Where-Object { $_.UserType -eq 'Guest' -or $_.OfficeLocation -eq $null}

# Create an array to store user data
$NoLicenseUser = @()
$groupNames = @()

foreach ($user in $ExternalUser) {
    ...
    foreach ($membershipItem in $membership) {
        ...
        $Allgroupnames = $groupNames -join ', '
        $NoLicenseUser += [PSCustomObject]@{
            "User"             = $user.DisplayName
            "UPN"              = $user.UserPrincipalName
            "Group"            = $Allgroupnames  # Updated to include the group's display name
