...
$outputFilePath = "C:\Users\GregorySemedo\Desktop\AllSales.csv"
$sharedMailboxFilePath = "C:\Users\GregorySemedo\Desktop\SharedMailboxAllSales.csv"
...
ForEach ($groupNoMember in $groupsNoMember) {
  ...
  $table += [PSCustomObject]@{
    ...
    GroupType = "GroupWithoutMember"
  }
}

# Afficher les groupes non-membres et leur propriétaire
ForEach ($groupNonMember in $groupsNonMember) {
  # Obtenir le propriétaire du groupe
  $owner = Get-AzureADGroupOwner -ObjectId $groupNonMember.ObjectId -All $true
  
  # Ajouter le nom du groupe et le nom du propriétaire à la table
  $table += [PSCustomObject]@{
    GroupName = $groupNonMember.DisplayName
    OwnerName = $owner.DisplayName
    GroupType = "GroupNonMember"
  }
}
...
$UserList = Import-Csv -Path "C:\Test\AllSales-SharedMailbox.csv"
