...
If(test-path $Minidump_Folder)
	{
		$Last_DMP = Get-Childitem $Minidump_Folder | Where-Object {$_.Extension -eq ".dmp"} | Sort-Object -Descending -Property LastWriteTime | Select-Object -First 1
		If($null -ne $Last_DMP)
			{
				...
				If($Last_DMP_delay -le $Delay_alert)
					{
						...
						$Get_BugCheck_Events = (Get-EventLog system -EntryType Error)
