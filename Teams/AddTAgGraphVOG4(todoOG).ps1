...
foreach ($Team in $AllTeamUser) {
    ...
    do {
        ...
        # Ask if want to add to another tag
        $AddToAnotherTag = Read-Host -Prompt "Do you want to add the user to another tag in Team $($Team.DisplayName)? (Y/N)"
    } while ($AddToAnotherTag -eq 'Y')
}
